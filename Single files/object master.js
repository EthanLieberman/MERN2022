...
const divideIdBy3 = pokémon.filter(p => p.id % 3 === 0);
// console.log(divideIdBy3)

const fireType = pokémon.filter(p => p.types.includes("fire") );
// console.log(fireType)

const over1Type = pokémon.filter(p => p.types.length > 1);
// console.log(over1Type)
...
const onlyPoison = pokémon.filter(p => p.types.length == 1).filter(p => p.types.includes("poison"))
